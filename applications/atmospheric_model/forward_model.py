...
dxy = 100  # resolution of the model in both x and y directions #####SLSLSL####
# dxy=100          # resolution of the model in both x and y directions #####SLSLSL####
dz = 10
x = np.mgrid[-2500:2500 + dxy:dxy]  # solve on a 5 km domain #####SLSLSL####
# x=np.mgrid[-25000:25000+dxy:dxy] # solve on a 5 km domain #####SLSLSL####
...
stack_x = [1500., 1000., -1000., 1250]
stack_y = [0., 1000., -1000., -2000.]
stacks = len(stack_x)

Q = 40. * np.ones(stacks)  # mass emitted per unit time
H = 50. * np.ones(stacks)  # stack height, m
...
wind_speed = 5. * np.ones((days * 24, 1))  # m/s  ####SLSLSL####
if wind == CONSTANT_WIND:
    wind_dir = 330. * np.ones((days * 24, 1))  ####SLSLSL####
    ...
    wind_dir = 0.5 * wind_dir + 0.5 * 330. * np.ones((days * 24, 1))
...
heat_vals = np.mean(C1, axis=2) * 1e6
heat_max = np.max(heat_vals)
outer_list = []

start_lat = 50.762406  # y
start_lon = 7.101995  # x
del_lat = 0.001525 - 0.002418
del_lon = 0.103448 - 0.101995
...
for i in range(0, rows):
    for j in range(0, cols):
        box_dict = {
            'type': 'Feature',
            'properties': {"color": heat_vals[i][j] / heat_max},
            'geometry': {
                'type': 'Polygon',
                'coordinates': [[
                    [(x[i][j] - x[0][0]) / dxy * del_lon + start_lon, (y[i][j] - y[0][0]) / dxy * del_lat + start_lat],
                    [(x[i][j] - x[0][0]) / dxy * del_lon + start_lon + del_lon,
                     (y[i][j] - y[0][0]) / dxy * del_lat + start_lat],
                    [(x[i][j] - x[0][0]) / dxy * del_lon + start_lon + del_lon,
                     (y[i][j] - y[0][0]) / dxy * del_lat + start_lat + del_lat],
                    [(x[i][j] - x[0][0]) / dxy * del_lon + start_lon,
                     (y[i][j] - y[0][0]) / dxy * del_lat + start_lat + del_lat]
                ]
                ]
            }
        }
        outer_list.append(box_dict)
...
#             'type':'Polygon',
#             'coordinates': [[
#                 [(y[i][j] - y[0][0]) / dxy * del_lat + start_lat, (x[i][j] - x[0][0]) / dxy * del_lon + start_lon],
#                 [(y[i][j] - y[0][0]) / dxy * del_lat + start_lat, (x[i][j] - x[0][0]) / dxy * del_lon + start_lon + del_lon],
#                 [(y[i][j] - y[0][0]) / dxy * del_lat + start_lat + del_lat, (x[i][j] - x[0][0]) / dxy * del_lon + start_lon + del_lon],
#                 [(y[i][j] - y[0][0]) / dxy * del_lat + start_lat + del_lat, (x[i][j] - x[0][0]) / dxy * del_lon + start_lon]
#             ]
#             ]
#             }
#         }
